...
class AdMob : CordovaPlugin() {
    ...
    private val actions = mapOf(
        ...
        Actions.OPEN_INSPECTOR to ::executeOpenInspector,
    ...
    private fun executeOpenInspector(ctx: ExecuteContext) {
        configForTestLabIfNeeded(ctx.activity)
        cordova.activity.runOnUiThread {
            MobileAds.openAdInspector(ctx.activity) { error ->
                if (error != null) {
                    ctx.reject(error.message)
                } else ctx.resolve()
            }
        }
    }
